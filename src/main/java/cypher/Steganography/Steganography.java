...
public class Steganography extends CypherAbstract {
    private static final String RESOURCES_PATH = "src/main/resources/steganography/";
    private final String normalImageName;
    private final String cryptedTextImageName;
     ...
     */
    private void encodeData(byte[] image, byte[] dataToAddList, int offset) {
        // check that the data + offset will fit in the image
        if ((dataToAddList.length * 8) + offset > image.length) {
            throw new IllegalArgumentException("File not long enough!");
        }

        for (int dataToAdd : dataToAddList) {
     ...
     */
    private void saveImage(BufferedImage image, File file, String ext) throws IOException {
